...
import pyodbc
from db_connection import load_dashboard_data

GITHUB_LINK = 'https://github.com/SafouaneHaddadi'

# Set up the Streamlit page config
st.set_page_config(
    page_title="Chicago Crime & Schools Dashboard",
    layout="wide",
...
with col1: #"within the first column"
    ...
    crimes_10yr = f"{int(data['Total_Crimes_2016_2025']):,}" # format with commas

    st.metric(
        label="Total Crimes (2016-2025)",
        value=crimes_10yr,
        delta=data['Crime_Change_5yr'], # change indicator
        delta_color="inverse"  # red if positive, green if negative
    )

with col2:
    crimes_2025 = f"{int(data['Crimes_In_2025']):,}"
    st.metric( #box with the label and value
        label="Crimes in 2025",
        value=crimes_2025,
    )
...
with col3:
    st.metric(
        label="Total Schools",
        value=f"{int(data['Total_Schools']):,}",
    )

with col4:
    st.metric(
        label="Avg Safety Score",
        value=f"{data['City_Avg_Safety_Score']}",
    )

with col5:
    st.metric(
        label="Avg Poverty Rate",
        value=f"{data['City_Avg_Poverty_Rate']}%",
    )


# =============================================================================

# COMMUNITY RISK VISUALIZATIONS
...
with col1:
    st.metric(
        ...
        value=data['High_Risk_Communities'],
        help="Communities needing immediate attention"
    )
    st.progress(data['High_Risk_Communities']/77, 
                text=f"{data['High_Risk_Communities']}/77 ({data['High_Risk_Communities']/77*100:.1f}%)")

with col2:
    st.metric(
        label="Critical Risk Communities",
        value=data['Critical_Risk_Communities'],
        help="Zero critical communities means we prevented the worst"
    )
    st.progress(data['Critical_Risk_Communities']/77, 
                text=f"{data['Critical_Risk_Communities']}/77 (0%)")
...
df_scatter = load_dashboard_data('vw_CommunityRisk')
...
if not df_scatter.empty:
    ...
    fig = px.scatter(
        ...
        y='total_crimes',
        color='risk_category',
        size='total_crimes',                 # bigger dots = more crime
...
with col1:
    ...
    fig = px.bar(
        ...
        text='Safety Score',
        title=f"School Safety Scores (Gap: {data['Safety_Gap']:.1f} points)"
    )
...
with col2:
    ...
    fig = px.bar(
        ...
        color_discrete_map={'High-Crime Areas': 'coral', 'Low-Crime Areas': 'lightgreen'},
        text='Math Score (%)',
        title=f"Math Proficiency (Gap: {data['Math_Gap']} points)"
    )
    fig.update_traces(textposition='outside')
    fig.update_layout(height=500)
    st.plotly_chart(fig, width='stretch')
